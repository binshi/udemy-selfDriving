    ...
    "environment": [
      {
        ...
        "value": "udacity-shibin-azure-sql"
      },
      {
        "name": "AZURE_DOTNET_APP",
        "value": "udacity-shibin-azure-dotnet-app"
      }
    ],
    "portMappings": [
      {
...
# Amazone S3 Bucket creation
resource "aws_s3_bucket" "shibins3" {
  bucket = "udacity-shibin-aws-s3-bucket"
}

resource "aws_s3_bucket_acl" "bucket_acl" {
  bucket = aws_s3_bucket.shibins3.id
  acl    = "private"
}

# Dynamo table
resource "aws_dynamodb_table" "udacity-shibin-aws-dynamodb" {
  name           = "GameScores"
